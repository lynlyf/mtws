function cmap = sealand(n,opt)
%% return colormap for gmt topography
% usage:
%   cmap = sealand([n,]'sea|land') 
%   cmap = sealand([n,][zlo,zhi]) 
%   
% input:
%   n: number of colors
%   opt: 'sea|land|sealand' | [zlo,zhi]
% output: colormap [r,g,b]
%
% see also: cmstore
%%
if nargin==0, [n,opt] = deal([],'sealand'); end
if nargin==1 
    if isnumeric(n) && isscalar(n) % (n)
        opt = 'sealand';
    else % ('sea|land|sealand'|[zlo,zhi])
        [n,opt] = deal([],n); 
    end
end
if isempty(n), n = cmnumber; end
if isempty(opt), opt = 'sealand'; end
...
if isnumeric(opt)
    if isscalar(opt) % zmx
        [zmin,zmax] = deal(-opt,opt);             
    else % [zlo,zhi] | z data
        [zmin,zmax] = getbound(opt);
    end
    assert(zmin<zmax,'zmin should be lower than zmax');
    if zmin > 0 % all land
        cmap = interp1(1:nlc,land,linspace(1,nlc,n));
    elseif zmin == 0 % sea surface and land
        cmap = [sea(end,:); interp1(1:nlc,land,linspace(1,nlc,n-1))];
    elseif zmax <= 0 % all sea
        cmap = interp1(1:nlc,land,linspace(1,nlc,n));
    else % sea and land
        m = ceil(-zmin/(zmax-zmin)*n);
        cmap = [interp1(1:nsc,sea,linspace(1,nsc,m));
                interp1(1:nlc,land,linspace(1,nlc,n-m))];
    end
    return;
end
...
switch(lower(opt))
    case {'s','sea'}
        cmap = interp1(1:nsc,sea,linspace(1,nsc,n));
    case {'l','land'}
        cmap = interp1(1:nlc,land,linspace(1,nlc,n));
    otherwise
        m = ceil(n/2);
