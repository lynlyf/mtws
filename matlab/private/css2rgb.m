% css2rgb  Rgb triple for given CSS color name (renamed from rgb.m)
%   https://cn.mathworks.com/matlabcentral/fileexchange/24497-rgb-triple-of-color-name--version-2
%
%   RGB = css2rgb('COLORNAME') returns the red-green-blue triple corresponding
...
%
%   EXAMPLES
%     c = css2rgb('DarkRed')               gives c = [0.5430 0 0]
%     c = css2rgb('Green')                 gives c = [0 0.5 0]
%     plot(x,y,'color',css2rgb('orange'))  plots an orange line through x and y
%     css2rgb('chart')                        shows all the colors
%
%   BACKGROUND
...
function rgb = css2rgb(s)
  persistent num name
  if isempty(num) % First time css2rgb is called
    [num,name] = getcolors();
    name = lower(name);
  ...
  if nargin==0, s = 'chart'; else, s = lower(s); end
  if ismember(s,{'show','chart'})
    showcolors()
  elseif ismember(s,{'list','name'})
    fprintf('=== %s-supported color names:\n%s\n===\n',mfilename,strjoin(sort(name),', '));
  else
